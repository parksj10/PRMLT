function [gamma, xi, c] = hmmSmoother(O, lambda)
...
xi = zeros(N,N,T-1); %also called "di-gamma"
gamma = zeros(N,T);
for t = 1:T-1
    for i = 1:N
        gamma(i,t) = 0;
        for j = 1:N
            xi(i,j,t) = alpha(i,t)*A(i,j)*B(j,O(t+1))*beta(j,t+1);
            gamma(i,t) = gamma(i,t) + xi(i,j,t);
        end %j
    end %i
end %t
